...
try:
    ...
    from gas_disaggregation import (
        disaggregate_to_gas_vector,
        get_baseline_gas_vector,
        CONTROLLED_GAS_NAMES,
    )
...
class ClimateMARL(MultiAgentEnv):
    ...
    def __init__(self, env_config, emission_data, economics_config, actions_config):
        ...
        if self.policy_matrix.shape != (self.N, self.lever_count, self._controllable_gases):
            raise ValueError(
                ...
                f"got {self.policy_matrix.shape}, expected "
                f"({self.N}, {self.lever_count}, {self._controllable_gases})"
            )
        ...
        self.max_prevention_benefit = float(economics_config.get("max_prevention_benefit", 0.5))
    ...
    def _make_net_engine(self):
        ...
        model  = instantiate_model(
            p["model_type"], p["n_gas"], p["hidden"], p["num_layers"],
            kernel_size=p.get("kernel_size", 3), device=device,
        )
        load_state_dict(model, numpy_state_to_torch(p["state_dict_np"], device))
    ...
    def _current_observation(self):
        ...
        year_idx_norm = year_idx / self.horizon
    ...
    def reset(self, *, seed=None, options=None):
        ...
        if _CO2_PREDICTOR_AVAILABLE and self.country_names:
            ...
            try:
                self.baseline_gas_vector = get_baseline_gas_vector(
                    self._hist_emissions_pd, year=self.hist_end
                )
            except Exception:
                self.baseline_gas_vector = None

        self.dynamic_co2        = np.zeros(self.N, dtype=np.float32)
        self.dynamic_gas_vector = np.zeros(
            (self.N, len(CONTROLLED_GAS_NAMES)), dtype=np.float32
        )
    ...
    def step(self, action_dict):
        ...
        self.prevention_stock = np.clip(
            self.prevention_stock * self.prevention_decay + adaptation_selected,
            0.0, self.max_prevention_benefit,
        )
        ...
        delta_growth              = np.einsum('ni,nij->nj', lever_efforts, self.policy_matrix)
        ...
        base_agent_year    = self.baseline_emissions_agent[:, idx + 1, :]
        delta_from_baseline = (
            emission_agents[:, self.control_indices]
            - base_agent_year[:, self.control_indices]
        ).astype(np.float32)
        ...
        if _CO2_PREDICTOR_AVAILABLE and self.country_names:
            action_keys = ["e", "m", "l"]
            for i, country in enumerate(self.country_names):
                if self.country_states.get(country) is None:
                    continue
                actions = {
                    action_keys[j]: float(lever_efforts[i, j])
                    for j in range(min(self.lever_count, 3))
                }
                actions["p"] = float(adaptation_selected[i])
                try:
                    self.country_states[country] = apply_action(
                        country, self.country_states[country],
                        actions, stochastic=False,
                    )
                    self.dynamic_co2[i] = predict_co2(
                        country, self.country_states[country]
                    )
                    if self.baseline_gas_vector is not None:
                        self.dynamic_gas_vector[i] = disaggregate_to_gas_vector(
                            country,
                            self.country_states[country],
                            float(self.dynamic_co2[i]),
                            self.baseline_gas_vector,
                        )
                except Exception:
                    self.dynamic_co2[i]        = 0.0
                    self.dynamic_gas_vector[i] = np.zeros(
                        len(CONTROLLED_GAS_NAMES), dtype=np.float32
                    )
        ...
        if done:
            ...
            for _ in range(self.rollout_length):
                idx                       = self.year_idx - (self.hist_end + 1)
                baseline_growth_year      = self.baseline_emission_growth[idx]
                baseline_growth_per_agent = np.broadcast_to(
                    baseline_growth_year, (self.N, self.G)
                ).copy()
            ...
            if self.env_config["log_episode_trajectories"]:
                years_arr    = np.asarray(self._years_log, dtype=np.int16)
                E_global_arr = np.stack(self._E_global_log, axis=0)
                T_arr        = np.asarray(self._temp_log, dtype=np.float32)
                log_dir      = Path(self.env_config["output_dir"]) / Path(self.env_config["log_dir"])
                os.makedirs(log_dir, exist_ok=True)
                ep_id = f"ep_{np.random.randint(1_000_000_000)}"
                np.savez_compressed(
                    os.path.join(log_dir, f"{ep_id}.npz"),
                    years=years_arr,
                    E_global=E_global_arr,
                    T=T_arr,
                    gas_names=np.array(self.gas_names, dtype=object),
                    dynamic_co2=self.dynamic_co2,
                    dynamic_gas_vector=self.dynamic_gas_vector,
                )
